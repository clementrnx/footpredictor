...
# --- CONFIGURATION CLEMENTRNXX PREDICTOR V5.8 ---
st.set_page_config(page_title="Clementrnxx Predictor V5.8", layout="wide")
...
st.markdown("""
    ...
    div.stButton > button {
        ...
        border-radius: 15px !important; font-weight: 900; width: 100%;
    }
...
# --- UI ---
st.title("⚽ CLEMENTRNXX PREDICTOR V5.8")

tab1, tab2 = st.tabs(["🎯 ANALYSE 1VS1", "📡 SCANNER ÉLITE"])

with tab1:
    # (Interface 1vs1 identique à la version précédente avec tous les champs)
    ...
    last_n_1v1 = col_n.number_input("DERNIERS MATCHS", 5, 50, 15)
    
    # ... Logique 1vs1 existante ...
...
with tab2:
    ...
    last_n_scan = sc5.number_input("PROFONDEUR DATA (LAST N)", 5, 50, 15)
    ...
    if st.button("🔥 GÉNÉRER TICKET ÉLITE & DISCORD"):
        ...
        with st.spinner("Sniping des opportunités..."):
            for lid in lids:
                                    ...
                                    if p_val >= cfg['p_min']:
                                        ...
                                        if score >= cfg['elite_min']:
                                            opps.append({
                                                ...
                                                "Pari": o['value'], "Cote": cote, "Prob": p_val, "Score": score
                                            })

        # Filtrage par limite de matchs (on prend les meilleurs scores en priorité)
        ...
        if final:
            ...
            t_msg += "\n".join([f"🔹 {x['Match']} : **{x['Pari']}** @{x['Cote']} ({x['Prob']:.1%})" for x in final])
            ...
            st.success(f"✅ Ticket de {len(final)} matchs envoyé sur Discord !")
        else:
            st.warning("Aucune opportunité ne correspond à ces critères.")
