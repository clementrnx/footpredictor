...
st.markdown("""
    ...
    h1, h2, h3, p, span, label { 
        ...
        letter-spacing: 2px;
        background: none !important; /* Enlève l'effet bulle derrière les textes */
    }

    /* BOUTON GLASS LONG SANS BRILLANCE EXCESSIVE */
    div.stButton > button {
    ...
    /* CHAMPS DE SAISIE ÉPURÉS (SANS BRILLANCE) */
    ...
    /* CARTE BET NETTOYÉE (SANS BARRE LATÉRALE) */
    .bet-card {
...
l_name = st.selectbox("CHOISIR LA LIGUE", ["Champions League", "Premier League", "La Liga", "Serie A", "Bundesliga", "Ligue 1"])
...
if st.session_state.simulation_done:
    ...
    c_h, c_n, c_a = b_c2.number_input(f"COTE {d['t_h']}", 2.0), b_c3.number_input("COTE NUL", 3.0), b_c4.number_input(f"COTE {d['t_a']}", 3.0)

    dc_1, dc_2, dc_3 = st.columns(3)
    c_hn, c_na, c_ha = dc_1.number_input(f"{d['t_h']}/NUL", 1.30), dc_2.number_input(f"NUL/{d['t_a']}", 1.30), dc_3.number_input(f"{d['t_h']}/{d['t_a']}", 1.30)
    ...
    if best_o['p'] * best_o['c'] > 1.02:
        ...
        m_finale = min(max(bankroll * 0.30, bankroll * k_val * 0.5), bankroll * 0.25)
